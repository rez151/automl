...
from keras.layers import Conv2D, MaxPooling2D, BatchNormalization, Flatten, Dense, Dropout, AveragePooling2D, \
    MaxoutDense, LeakyReLU


def load_2nd_place(width, height, classes_num):
    with tf.device('/cpu:0'):
        model = Sequential()
        model.add(Conv2D(32, 5, strides=2, activation='relu', padding='same', input_shape=(width, height, 3)))
        model.add(Conv2D(32, 3, strides=2, activation='relu', padding='same'))
        model.add(MaxPooling2D(pool_size=3, strides=2))
        model.add(Conv2D(64, 5, strides=2, activation='relu', padding='same'))
...
def load_usuyama(width, height, classes_num):
    with tf.device('/cpu:0'):
        model = Sequential()
...
def load_m42(width, height, classes_num):
    with tf.device('/cpu:0'):
        model = Sequential()
        model.add(Conv2D(32, 4, padding='same', input_shape=(width, height, 3)))
        model.add(LeakyReLU())
        ...
        model.add(MaxPooling2D(pool_size=3, strides=2))
        model.add(LeakyReLU())
        model.add(Conv2D(384, 4, padding='same'))
        model.add(LeakyReLU())
        model.add(Conv2D(384, 4, padding='same'))
        model.add(LeakyReLU())
        model.add(MaxPooling2D(pool_size=3, strides=2))
...
def load_alexnet(width, height, classes_num):
    ...
    with tf.device('/cpu:0'):
        model = Sequential()
        model.add(
            Conv2D(96, (11, 11), strides=(4, 4), activation='relu', padding='same', input_shape=(None, None, 3)))
